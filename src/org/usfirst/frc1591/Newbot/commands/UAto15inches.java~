 ...
 *
 */
public class UAto15inches extends Command {
	static final double INCHES = 0.215;
    ...
    protected void initialize() {
    	...
    	// Check if above or below 15" point, reads position
    	...
    	if (leftPos > INCHES) {
    		...
    		goingDown = false;
    		Robot.leftArm.leftarmUp(SPEED);
    		Robot.rightArm.rightarmUp(SPEED);
    	}
    }
    ...
    protected boolean isFinished() {
        if (goingDown) {
        	if (Robot.leftArm.getPosition() < INCHES)
        		return true;
        } else {
        	if (Robot.leftArm.getPosition() > INCHES)
        		return true;
